...
SCALE="${2:?candidate value scale required}"
...
run_match() {
  ...
  DISTILL_VARIANT_NAME="${MODEL}-scale-${SCALE}-vs-1000" \
  ...
  DISTILL_CANDIDATE_VALUE_SCALE_CP="$SCALE" \
  DISTILL_OPPONENT_VALUE_SCALE_CP=1000 \
...
DISTILL_RUN_ID="$RUN_ID" \
DISTILL_MODEL_NAME="$MODEL" \
DISTILL_VARIANT_NAME="${MODEL}-scale-${SCALE}-vs-1000-clock" \
DISTILL_OPPONENT_MODEL_NAME="$MODEL" \
DISTILL_CANDIDATE_ANTISYMMETRIC=1 \
DISTILL_OPPONENT_ANTISYMMETRIC=1 \
DISTILL_CANDIDATE_SEARCH=strong \
