...
MODEL_NAME="${DISTILL_MODEL_NAME:-phase-m-h64-policy-1m}"
...
aws s3 cp "$RUN_PREFIX/models/$BASE_MODEL/$BASE_MODEL.npz" "$BASE_PATH" \
  --only-show-errors
"$PYTORCH_PYTHON" -m training.train_policy \
  --data "$DATASET_DIRECTORY" \
  --base "$BASE_PATH" \
  --output "$OUTPUT_PATH" \
  --device cuda \
  --epochs "${DISTILL_EPOCHS:-8}" \
  --batch-size "${DISTILL_BATCH_SIZE:-4096}" \
  --learning-rate "${DISTILL_LEARNING_RATE:-0.001}" \
  --policy-width "${DISTILL_POLICY_WIDTH:-32}" \
  --temperature "${DISTILL_POLICY_TEMPERATURE:-0.25}" \
  --seed 23 | tee "$WORK_DIRECTORY/training.log"
