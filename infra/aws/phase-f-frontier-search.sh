...
# Escalating gates avoid spending hours on a bundle that loses immediately.
run_match 1000 "${BENCH_1K_ROUNDS:-20}"
run_match 10000 "${BENCH_10K_ROUNDS:-5}"
if (( ${BENCH_100K_ROUNDS:-1} > 0 )); then
  run_match 100000 "${BENCH_100K_ROUNDS:-1}"
fi

DISTILL_RUN_ID="$RUN_ID" \
DISTILL_MODEL_NAME="$MODEL" \
DISTILL_VARIANT_NAME="${MODEL}-frontier-vs-strong-clock" \
DISTILL_OPPONENT_MODEL_NAME="$MODEL" \
DISTILL_CANDIDATE_ANTISYMMETRIC=1 \
DISTILL_OPPONENT_ANTISYMMETRIC=1 \
DISTILL_CANDIDATE_SEARCH=frontier \
