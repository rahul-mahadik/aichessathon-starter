...
- Illegal move, malformed output, crash, out of memory, or flag fall loses that game. A move
  reply over 4 KB counts as illegal. Games still running at 600 plies are drawn.
...
- Rated games start from curated opening positions, not the standard start. The set is not
  published.
- The process is suspended while the opponent thinks. State survives between our moves, but
  background pondering does not run during the opponent's clock.
